...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await commitRenterImport({
      rentalGroupId,
      uploadedByUserId,
      fileName: file.name,
      rawHeaders,
    ...
    console.error("POST /api/imports/renters/commit error:", error);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Không thể commit import người mặc",
      },
      { status: 500 }
    );
  }
